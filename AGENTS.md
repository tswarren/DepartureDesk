...
M1, M2, and M3 are complete. M4 is accepted and shipped through M4D.0. [M4D.1](docs/planning/m4d1-departure-composition-workspace.md) is accepted, and the Cruise vertical through Slice 2D is shipped. Composition is the primary Staff chrome for draft and active Departures with `manage_departures`. [M4D.0R](docs/planning/m4d0r-builder-interface-remediation.md) is historical interim presentation. [Slice 3R](docs/planning/m4d1-slice3r-non-cruise-adapter-boundary.md) is accepted for the layer boundary and delivery order only and authorizes no adapter code. Slice standing is [docs/planning/README.md](docs/planning/README.md). The next unauthorized boundary is the Hotel walkthrough. Hotel implementation remains prohibited until that walkthrough and a separate Slice 3A implementation plan are accepted. Do not implement Hotel, later non-Cruise adapters, M4E, or M5 until their accepted plans name that work. Unmerged branch `m4d1-slice3-hotel-transport-activity` and PR #156 are prototype evidence and are not an implementation source. The shipped domain is agency identity, the Client and Supplier directories, Departures, Supplier Arrangements through Arrangement ending, and Client offers through publication and the Cruise composition workspace. Travel Program, remittance, FX, Client demand, Travelers, Obligations, and Payments remain unimplemented until an accepted slice plan names that work. Celebrity deposits and milestones remain Arrangement-wide. Documentation authority is indexed in [docs/README.md](docs/README.md). Product authority is [docs/planning/departure-desk-mvp.md](docs/planning/departure-desk-mvp.md) and [docs/planning/commercial-domain-decision-register.md](docs/planning/commercial-domain-decision-register.md). Do not implement MFA, platform support, a workforce-role taxonomy, Travel Program, or later commercial records from those documents until an accepted slice plan names that work. An accepted milestone contract is not enough. [docs/terminology.md](docs/terminology.md) is the current vocabulary; Party-era terminology is archived.
...
Also run system tests for browser-visible workflows. The development image includes Chromium, so `./dev/rails-docker bin/rails test:system` and `bin/ci` run them locally. GitHub CI system tests remain required and blocking. `bin/rails tailwindcss:build` is a blocking step in the GitHub `test` and `system-test` jobs and in `bin/ci`.
