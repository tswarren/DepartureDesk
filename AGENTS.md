...
- [ADR 0014: Client offers, publication, and Supplier-source compatibility](docs/adr/0014-client-offers-publication-and-supply-compatibility.md) accepts Service Offer and Package identity, publication, activated-only M3 pins, Sales enabled versus computed eligibility, and the M5 Charge-posting handoff. It is not implementation authority. [M4.0](docs/planning/m40-task-flow-and-contract.md) is the accepted task-flow gate. Do not implement M4A–M4E until that slice's accepted plan names the work.
