...
- [ADR 0014: Client offers, publication, and Supplier-source compatibility](docs/adr/0014-client-offers-publication-and-supply-compatibility.md) accepts Service Offer and Package identity, publication, activated-only M3 pins, Sales enabled versus computed eligibility, and the M5 Charge-posting handoff. [M4A](docs/planning/m4a-service-definitions-and-sources.md) is the shipped implementing slice for unpublished drafts and source compatibility. It is not implementation authority for later slices. [M4.0](docs/planning/m40-task-flow-and-contract.md) is the accepted task-flow gate. Do not implement M4B–M4E until that slice's accepted plan names the work.
...
15. Audit successful administrative commands in the same transaction. Subjects are `Agency`, `AgencyUser`, `Office`, `ClientPerson`, `Client`, `ClientOrganization`, `Supplier`, `SupplierLocation`, `SupplierContact`, `Departure`, `SupplierArrangement`, `SupplierReservation`, and `ServiceOffer` only. Do not audit expected failures.
...
- Audit subjects are narrowly typed: an `Agency` subject must equal the event agency; an `AgencyUser`, `Office`, `Departure`, `SupplierArrangement`, `SupplierReservation`, or `ServiceOffer` subject must belong to it. Unknown subject types raise.
