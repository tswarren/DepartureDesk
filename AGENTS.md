...
The shipped domain is agency identity, the complete M1 Client and Supplier directories, [M2A](docs/planning/m2a-departure-core.md) Departure draft/activation/`D-` issuance/return to draft/search, [M2B](docs/planning/m2b-departed-lifecycle.md) departed transitions, scheduled lifecycle jobs, and schedule/currency/lifecycle corrections, [M2C](docs/planning/m2c-acceptance-and-hardening.md) proof and hardening, and [M3A](docs/planning/m3a-draft-arrangement-structure.md) draft Supplier Arrangement structure. M2 is complete. The [M3 parent](docs/planning/m3-supplier-planning.md) is accepted and is not implementation authority for later slices. Travel Program, Arrangement activation, capacity, costs, Reservations, commitments, Deadlines, exposure, and later M3 records remain unimplemented. Do not implement them until an accepted slice plan names that work.
...
- [ADR 0008: Supplier Arrangement version topology](docs/adr/0008-supplier-arrangement-version-topology.md) accepts stable Arrangement and child identities with per-version definitions. M3A ships the initial draft topology only; activation and successor versions remain later-slice work.
- [ADR 0009: Supplier contracting and service-provider roles](docs/adr/0009-supplier-contracting-and-service-provider-roles.md) accepts contracting Supplier, computed Service Provider, Arrangement contact, and inactivation-dependency meanings. M3A ships the dependencies and recovery behavior it introduces; later slices extend the same rules.
