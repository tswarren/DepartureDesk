...
The shipped domain is agency identity, the complete M1 Client and Supplier directories, [M2A](docs/planning/m2a-departure-core.md) Departure draft/activation/`D-` issuance/return to draft/search, [M2B](docs/planning/m2b-departed-lifecycle.md) departed transitions, scheduled lifecycle jobs, and schedule/currency/lifecycle corrections, [M2C](docs/planning/m2c-acceptance-and-hardening.md) proof and hardening, [M3A](docs/planning/m3a-draft-arrangement-structure.md) draft Supplier Arrangement structure, [M3B](docs/planning/m3b-supplier-capacity.md) draft Supplier capacity configuration plus the capacity event/projection/reconciliation engine, [M3C](docs/planning/m3c-cost-terms-and-forecasts.md) draft Supplier cost terms and derived forecasts, and [M3D.0](docs/planning/m3d0-planning-workspace-compression.md) guided workspace compression over M3A–M3C. M2 is complete. The [M3 parent](docs/planning/m3-supplier-planning.md) is accepted and amended through M3D authority. [M3D](docs/planning/m3d-activation-reservations-confirmations.md) and [ADR 0012](docs/adr/0012-arrangement-activation-reservations-and-confirmations.md) are Implemented—remediation pending (not yet re-marked Shipped). Arrangement activation, Reservations, confirmations, effective-capacity controls, and confirmation-triggered commitment openings are implemented M3D vocabulary under remediation. Travel Program, Deadlines, exposure, remittance, FX, Arrangement ending, and later M3E–M3F records remain unimplemented. Do not implement them until an accepted slice plan names that work. Do not start M3E until M3D is re-marked Shipped.

Documentation authority and status are indexed in [docs/README.md](docs/README.md). Product authority is [docs/planning/departure-desk-mvp.md](docs/planning/departure-desk-mvp.md) and [docs/planning/commercial-domain-decision-register.md](docs/planning/commercial-domain-decision-register.md). Do not implement MFA, platform support, a workforce-role taxonomy, Travel Program, or later commercial records from those documents until an accepted slice plan names that work. An accepted milestone contract is not enough. [M1A](docs/planning/m1a-individual-client.md), [M1B](docs/planning/m1b-client-organizations.md), [M1C](docs/planning/m1c-supplier-core.md), [M1D](docs/planning/m1d-supplier-locations-and-contacts.md), [M1E](docs/planning/m1e-directory-acceptance-and-hardening.md), [M2A](docs/planning/m2a-departure-core.md), [M2B](docs/planning/m2b-departed-lifecycle.md), [M2C](docs/planning/m2c-acceptance-and-hardening.md), [M3A](docs/planning/m3a-draft-arrangement-structure.md), [M3B](docs/planning/m3b-supplier-capacity.md), [M3C](docs/planning/m3c-cost-terms-and-forecasts.md), and [M3D.0](docs/planning/m3d0-planning-workspace-compression.md) are shipped. M2 is complete. [M3D](docs/planning/m3d-activation-reservations-confirmations.md) is Implemented—remediation pending. Do not implement Arrangement ending or other M3E–M3F records until an accepted slice plan names that work, and not before M3D is re-marked Shipped. [docs/terminology.md](docs/terminology.md) is the current vocabulary; Party-era terminology is archived.
...
- [ADR 0008: Supplier Arrangement version topology](docs/adr/0008-supplier-arrangement-version-topology.md) accepts stable Arrangement and child identities with per-version definitions. M3A draft topology is implemented; activation and successor behavior are Implemented—remediation pending with M3D.
...
- [ADR 0010: Supplier capacity ledger and projection](docs/adr/0010-supplier-capacity-ledger-and-projection.md) accepts stable Capacity Pool identity, immutable capacity events, scheduled effectiveness, reconciliation, and rebuildable projections. [M3B](docs/planning/m3b-supplier-capacity.md) shipped the implementing persistence and engine; Arrangement activation and Staff event UI are Implemented—remediation pending with M3D.
- [ADR 0011: Supplier cost definitions and forecast evaluation](docs/adr/0011-supplier-cost-definitions-and-forecast-evaluation.md) accepts exact-version cost sources, staged definitions, constrained components, planning assumptions, and derived forecasts. [M3C](docs/planning/m3c-cost-terms-and-forecasts.md) shipped the implementing slice; Arrangement activation and effective contracted terms are Implemented—remediation pending with M3D.
- [ADR 0012: Arrangement activation, Reservations, and confirmations](docs/adr/0012-arrangement-activation-reservations-and-confirmations.md) accepts activation manifests, successor copying, Reservation history, confirmation evidence, and the M3D confirmation-triggered commitment core. [M3D](docs/planning/m3d-activation-reservations-confirmations.md) is Implemented—remediation pending after shipped [M3D.0](docs/planning/m3d0-planning-workspace-compression.md).
...
15. Audit successful administrative commands in the same transaction. Subjects are `Agency`, `AgencyUser`, `Office`, `ClientPerson`, `Client`, `ClientOrganization`, `Supplier`, `SupplierLocation`, `SupplierContact`, `Departure`, and `SupplierArrangement` only. Do not audit expected failures.
