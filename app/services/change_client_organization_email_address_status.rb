class ChangeClientOrganizationEmailAddressStatus < ClientOrganizationContactPointCommand
  def call
    prepare!
    raise Error.new("Choose active or inactive.", code: :invalid) unless ClientOrganizationEmailAddress::STATUSES.include?(@status)

    ActiveRecord::Base.transaction do
      @agency.lock!
      organization = locked_organization
      raise Error.new("That organization is not active.", code: :invalid_state) if @status == "active" && organization.inactive?
      point = organization.email_addresses.lock.find(@record.id)
