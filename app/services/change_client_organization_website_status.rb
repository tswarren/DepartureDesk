class ChangeClientOrganizationWebsiteStatus < ClientOrganizationContactPointCommand
  def call
    prepare!
    raise Error.new("Choose active or inactive.", code: :invalid) unless ClientOrganizationWebsite::STATUSES.include?(@status)

    ActiveRecord::Base.transaction do
      @agency.lock!
      organization = locked_organization
      raise Error.new("That organization is not active.", code: :invalid_state) if @status == "active" && organization.inactive?
      point = organization.websites.lock.find(@record.id)
      ...
      audit_contact!(organization, record: point, changed_fields: [ "website_status" ])
