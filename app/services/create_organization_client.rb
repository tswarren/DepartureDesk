class CreateOrganizationClient < AgencyCommand
  include ClientReferenceIssuance
  COMMAND = "CreateOrganizationClient"
  ...
  def call
    ...
    ActiveRecord::Base.transaction do
      ...
      decision = acknowledge!(organization_id, client_id)
      ...
      audit_override!(organization, client, decision) if decision
  ...
  def acknowledge!(organization_id, client_id)
    DirectoryDuplicateGate.new(
      ...
      proposed_ids: { "organization_id" => organization_id, "client_id" => client_id }
    ).call { candidates }
  end
  ...
  def audit_override!(organization, client, decision)
    audit!(
      agency: @agency,
