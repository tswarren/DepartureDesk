class SupplierContactDestinationCommand < AgencyCommand
  def initialize(agency:, actor:, supplier:, supplier_contact:, record: nil, attributes: {}, lock_version: nil, acknowledgement_token: nil, acknowledgement_reason: nil, status: nil)
    @agency = agency
    @actor = actor
    @supplier = supplier
  ...
  def locked_contact(supplier = locked_supplier)
    @locked_contact ||= supplier.contacts.lock.find(@supplier_contact.id)
  end
  ...
  def audit_contact!(contact, record:, changed_fields:)
    audit!(
      agency: @agency,
      action: "supplier_contact.contact_updated",
      subject: contact,
      actor: @actor,
      details: {
  ...
  def audit_override!(contact, decision = nil)
    audit!(
      agency: @agency,
