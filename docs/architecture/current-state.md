...
DepartureDesk ships agency identity, administration, the complete M1 Client and Supplier directories (M1A-M1E), M2A Departure draft, activation, reference issuance, return to draft, and search, M2B departed transitions, scheduled departed jobs, and schedule/currency/lifecycle corrections, M2C proof and hardening, M3A draft Supplier Arrangement structure under Departures, M3B draft Supplier capacity configuration plus the capacity event/projection/reconciliation engine, M3C draft Supplier cost terms plus derived forecasts, and M3D.0 guided workspace compression over M3A–M3C. M2 is complete. [M3D](../planning/m3d-activation-reservations-confirmations.md) is shipped. Arrangement activation, Staff-facing effective capacity controls, Reservations, confirmations, and confirmation-triggered commitment openings are shipped. [M3D.7](../planning/m3d7-activated-definition-immutability.md) freezes exact-version definition graphs in Rails and PostgreSQL once an Arrangement version leaves draft. [M3D.8](../planning/m3d8-activation-reservation-product-quality.md) remediates activation and Reservation product-quality (forms, error recovery, progressive disclosure, composers, bounded lists, table overflow). [M3D.9](../planning/m3d9-reservation-integrity.md) remediates Reservation integrity (confirmed quantity-basis fidelity, successor revision/request revalidation, Capacity Pool definition membership, and event↔outcome compatibility). [M3E](../planning/m3e-supplier-operational-control.md) and [ADR 0013](../adr/0013-supplier-operational-commitments-deadlines-exposure-and-ending.md) are shipped through M3E.7b (including M3E.5R and M3D remediations): source-shaped openings and dispositions, Deadlines, Deposit Requirements and planning milestones, qualified exposure, Needs attention, and Arrangement ending. M3E is fully shipped and production-ready. [M4A](../planning/m4a-service-definitions-and-sources.md) and [M4B](../planning/m4b-client-pricing-and-anonymous-preview.md) are shipped. [M4C](../planning/m4c-packages-choices-and-client-terms.md) is shipped. [M4D](../planning/m4d-publication-and-live-feasibility.md) is shipped. [M4D.0](../planning/m4d0-narrow-group-departure-builder.md) and historical interim [M4D.0R](../planning/m4d0r-builder-interface-remediation.md) are shipped. [M4D.1 Slice 1](../planning/m4d1-slice1-workspace-foundation.md) is shipped (Composition primary Staff chrome; `/builder` redirect). [M4D.1 Slice 2A.1](../planning/m4d1-slice2a1-cruise-sailing-and-cabin-inventory.md) is shipped (typed Cruise sailing and cabin inventory). The Cruise vertical through [M4D.1 Slice 2D](../planning/m4d1-slice2d-cruise-client-terms-and-scenario-review.md) is shipped, including [Slice 2C](../planning/m4d1-slice2c-cruise-service-connection.md) (merge `b35a4f6`) and Slice 2D (merge `dc272a3`, PR #154). [M4D.1](../planning/m4d1-departure-composition-workspace.md) remains Accepted for later slices. Later commercial records are not shipped. The MVP and commercial decision register describe future product behavior; they are not claims about current persistence or routes.
...
| `ServiceOffer`, `ServiceOfferVersion` | M4A unpublished Client-offer identity under one Departure, with numbered versions. M4A drafts use `draft`/`abandoned`; M4D ships `published`/`superseded`/`retired`, current published pointers, sales states, and publication manifests. No generated reference and no identity-level sellability flag. |
...
| `Package`, `PackageVersion` | M4C unpublished Package identity under one Departure, with numbered versions. M4C drafts use `draft`/`abandoned`; M4D ships Package publication, freeze, Sales enabled, and live feasibility. Package-only ownership is `owning_package_version_id` on a Service Offer version. |
