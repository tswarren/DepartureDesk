...
DepartureDesk ships agency identity, administration, the complete M1 Client and Supplier directories (M1A-M1E), M2A Departure draft, activation, reference issuance, return to draft, and search, M2B departed transitions, scheduled departed jobs, and schedule/currency/lifecycle corrections, M2C proof and hardening, M3A draft Supplier Arrangement structure under Departures, M3B draft Supplier capacity configuration plus the capacity event/projection/reconciliation engine, M3C draft Supplier cost terms plus derived forecasts, and M3D.0 guided workspace compression over M3A–M3C. M2 is complete. [M3D](../planning/m3d-activation-reservations-confirmations.md) is Accepted and not yet shipped. Arrangement activation, Staff-facing effective capacity or event controls, effective contracted terms, Reservations, confirmations, commitment openings, and later commercial records are not shipped. The MVP and commercial decision register describe future product behavior; they are not claims about current persistence or routes.
