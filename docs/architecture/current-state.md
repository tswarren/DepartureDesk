...
| `AuditEvent` | Append-only evidence for supported Agency, AgencyUser, Office, ClientPerson, Client, ClientOrganization, Supplier, SupplierLocation, SupplierContact, Departure, SupplierArrangement, SupplierReservation, and ServiceOffer commands. |
...
| `AgencyCommandIdempotencyKey` | Agency-scoped replay guard for idempotent M3A create commands, M3B Pool create / capacity event / reconciliation commands, M3C cost create commands, and M4A Service Offer create/binding commands, keyed by command name, client idempotency key, and payload digest. |
...
| `ServiceOfferSourceBinding` | Exact M3 source pin (Arrangement/Item plus optional Occurrence, Resource, and Pool) with required/alternative topology, dependency flags, and Client-text provenance. |
