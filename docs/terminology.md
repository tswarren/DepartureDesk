...
| Supplier Arrangement | M3A draft vocabulary implemented on the M3A branch. The Agency's agreement or planning relationship with one contracting Supplier for one Departure. Lifecycle is `draft`, `active`, `ended`, or `abandoned`. M3A creates only `draft` and `abandoned` arrangements. There is no Arrangement `cancelled` status. |
| Arrangement version | M3A draft vocabulary implemented on the M3A branch. Exact commercial definition of a Supplier Arrangement. Activated versions are immutable; successor versions govern future resolution without rewriting prior use. Lifecycle is `draft`, `activated`, `superseded`, or `abandoned`. M3A creates only the initial `draft` version and may abandon it. |
| Arrangement Item | M3A draft vocabulary implemented on the M3A branch. A separately described Supplier-side service, deliverable, or commercial line within a Supplier Arrangement. |
| Service Occurrence | M3A draft vocabulary implemented on the M3A branch. A dated or otherwise bounded performance of an Arrangement Item. Dates may fall outside the Departure operating window. Lifecycle belongs to the stable Occurrence identity for planning and inactivation as `planned` or `cancelled`. There is no Occurrence `completed` status. |
| Supplier Resource | M3A draft vocabulary implemented on the M3A branch. The supplied unit or category relevant to capacity or later placement, such as a cabin category, room type, or coach. |
