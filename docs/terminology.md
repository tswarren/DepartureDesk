...
| Cruise deposits and deadlines workspace | Shipped [M4D.1 Slice 2B](planning/m4d1-slice2b-cruise-deposits-deadlines-and-activation-safe-editing.md) vocabulary (typed deadlines, deposits, activation preview, active operational chrome, milestone recording, and successor compare). Typed Arrangement-level adapter over generic M3E Deadline and Deposit Requirement definitions for Stop point D; not a parallel payment or obligation model. Shipped [M4D.1 Slice 2B-UX](planning/m4d1-slice2bux-deposits-deadlines-workspace-remediation.md) remediates that workspace task flow; [Slice 2B-UX-R](planning/m4d1-slice2buxr-contributor-replace-and-closure.md) closes contributor-replace collisions. |
| Cruise service connection | Shipped [M4D.1 Slice 2C](planning/m4d1-slice2c-cruise-service-connection.md) vocabulary. One Client Service Offer for one Cruise Arrangement Item, with cabin categories as an exactly-one choice and an option-owned Client rate-category key. Not a parallel Cruise Service model and not a Client price. |
| Cruise Client terms | Shipped [M4D.1 Slice 2D](planning/m4d1-slice2d-cruise-client-terms-and-scenario-review.md) vocabulary (merge `dc272a3`, PR #154). Category-scoped Client price components for one connected Cruise Service, optional Supplier-copy provenance, and write-free Single, Double, and supported Triple review. Not a second price definition and not a live Supplier link. |
| Supplier Composition, Offer Design, and Client Trip | Accepted [M4D.1 Slice 3R](planning/m4d1-slice3r-non-cruise-adapter-boundary.md) vocabulary for the three layers. Supplier Composition is what was contracted. Offer Design is what the Agency is prepared to sell. Client Trip is what a Client booked. Included versus optional placement is Offer Design. The next unauthorized boundary is the Hotel walkthrough. That walkthrough does not authorize code. |
