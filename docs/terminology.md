...
| Supplier Reservation | Accepted M3D vocabulary (not yet shipped). Stable Supplier-facing booking identity under an M3 Arrangement, with exact-version revisions and precise scope rows. |
...
| Capacity event | Shipped M3B engine vocabulary. Append-only Supplier-side capacity fact (`established`, `increased`, `released`, `reinstated`, `withdrawn`, `corrected_up`, `corrected_down`). Not reachable from Staff UI until M3D ships. |
...
| Capacity reconciliation | Shipped M3B engine vocabulary. Observed Supplier quantity compared to the ledger at a point in time, with append-only resolution. Not reachable from Staff UI until M3D ships. |
...
| Forecast readiness | Shipped M3C draft vocabulary. Explicit `forecast_ready` state on a cost definition after validation (and Staff attestation for contracted). Planning claim only; not effective contracted terms. Arrangement activation and effective contracted terms remain Accepted M3D. |
