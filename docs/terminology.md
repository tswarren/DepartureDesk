...
| Service Offer | Accepted M4 vocabulary; unimplemented until an M4A–M4E slice names that work. Departure-owned sellable-service identity with immutable numbered versions. Distinct from Arrangement Item and from later Client Trip Service. |
| Package | Accepted M4 vocabulary; unimplemented until an M4A–M4E slice names that work. Optional Departure-owned offer container with immutable numbered versions. An exact Package version pins exact Service Offer versions and may carry a bundled or service-sum Client price. |
| Sales enabled | Accepted M4 vocabulary; unimplemented until an M4A–M4E slice names that work. Staff intent that an exact published Package or Service Offer version is available for ordinary pause/resume. It is not computed eligibility and does not edit published terms. |
| Selectable now / On request / Unavailable | Accepted M4 vocabulary; unimplemented until an M4A–M4E slice names that work. Computed selection label with a reason, derived from dates, lifecycle, source, choices, and capacity. Distinct from Sales enabled. |
