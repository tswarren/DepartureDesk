...
| Commitment | Shipped M3D opening vocabulary for confirmation-triggered openings. Accepted M3E extends openings to source-shaped kinds (`confirmation_trigger`, `deposit_requirement` tranche, `deadline_requirement`) with append-only dispositions including `released`; there is no unrestricted manual opening. Not yet implemented beyond M3D. Explicit contractual exposure that may precede a Supplier Obligation. It is not an Obligation, invoice, or Payment. |
...
| Deadline | Accepted M3E vocabulary (not yet implemented). Resolved operational due fact linked to its governing source; actionable or informational. |
