...
| Sales enabled | Accepted [M4D](planning/m4d-publication-and-live-feasibility.md) vocabulary; not shipped. Staff intent that an exact published Package or Service Offer version is available for ordinary pause/resume. It is not computed eligibility and does not edit published terms. |
| Selectable now / On request / Unavailable | Accepted [M4D](planning/m4d-publication-and-live-feasibility.md) vocabulary; not shipped. Computed selection label with a reason, derived from dates, lifecycle, source, choices, and capacity. Distinct from Sales enabled. |
