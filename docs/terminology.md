...
| Capacity event | Shipped M3B engine vocabulary. Append-only Supplier-side capacity fact (`established`, `increased`, `released`, `reinstated`, `withdrawn`, `corrected_up`, `corrected_down`). Not reachable from Staff UI until M3D. |
...
| Capacity reconciliation | Shipped M3B engine vocabulary. Observed Supplier quantity compared to the ledger at a point in time, with append-only resolution. Not reachable from Staff UI until M3D. |
| Supplier cost source | Accepted M3C vocabulary (not yet shipped). Exact-version economic identity for one Supplier cost, Arrangement-wide or Item-scoped, with an explicit charging Supplier. |
| Supplier cost definition | Accepted M3C vocabulary (not yet shipped). Editable estimate or contracted term definition (`working` / `forecast_ready`) for one source. Currency equals Departure `operating_currency`. |
| Cost component | Accepted M3C vocabulary (not yet shipped). Typed component with economic role separate from calculation kind; explicit quantity and percentage-base semantics. |
| Charging Supplier | Accepted M3C vocabulary (not yet shipped). Explicit Supplier expected to charge the Agency for a cost source; independent from Service Provider. |
| Cost usage assumption | Accepted M3C vocabulary (not yet shipped). Lightweight exact Item-context planning quantities and optional anonymous occupancy profiles. Not Client demand. |
| Forecast readiness | Accepted M3C vocabulary (not yet shipped). Explicit `forecast_ready` state on a cost definition after validation (and Staff attestation for contracted). Planning claim only; not effective contracted terms. |
