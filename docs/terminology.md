...
| Cruise Client terms | Accepted [M4D.1 Slice 2D](planning/m4d1-slice2d-cruise-client-terms-and-scenario-review.md) vocabulary. Category-scoped Client price components for one connected Cruise Service, optional Supplier-copy provenance, and write-free Single, Double, and supported Triple review. Not a second price definition, not a live Supplier link, and not shipped. |
| Hotel, Transportation, and Activity adapters | Accepted [M4D.1 Slice 3](planning/m4d1-slice3-hotel-transportation-activity-adapters.md) vocabulary. Typed Supplier setup and Service connection for lodging, ground transportation, and activity, meal, or excursion Items, plus a mixed-DMC Item table. Not a DMC model, not a Client-price matrix, and not shipped. |
