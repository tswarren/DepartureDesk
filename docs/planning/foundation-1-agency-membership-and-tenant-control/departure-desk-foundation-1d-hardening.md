...
- Recovery `reactivate` delegates lock ownership to that primitive and writes `administrator_recovery_started` only after those locks. `replace_invitation` and `invite_replacement` keep the agency-first transaction.
