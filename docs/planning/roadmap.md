...
| M4 — Offers and pricing | Parent accepted; shipped through M4D.0; M4D.1 in progress. See the [planning index](README.md). | Describe Packages and standalone services the Agency intends to sell. The [parent](m4-offers-and-pricing.md), [ADR 0014](../adr/0014-client-offers-publication-and-supply-compatibility.md), and [M4.0](m40-task-flow-and-contract.md) are accepted and are not implementation authority for later slices. The next unauthorized boundary is the Hotel walkthrough. Hotel implementation remains prohibited until that walkthrough and a separate Slice 3A implementation plan are accepted. |
...
6. [M4](m4-offers-and-pricing.md) standing, including M4D.1, is the [planning index](README.md). The next unauthorized boundary is the Hotel walkthrough. Hotel implementation remains prohibited until that walkthrough and a separate Slice 3A implementation plan are accepted. M4E remains unimplemented until an accepted M4E plan names that work. Travel Program, remittance, FX, Client demand, Travelers, Obligations, and Payments remain unimplemented. Celebrity deposits and milestones remain Arrangement-wide; `per_source` Deadlines remain deferred.
