...
**Prerequisites:** Accepted [M4 parent](../m4-offers-and-pricing.md), [ADR 0014](../../adr/0014-client-offers-publication-and-supply-compatibility.md), and [M4.0](../m40-task-flow-and-contract.md) (Accepted 2026-09-20; not implementation authority for M4A–M4E). MVP occupancy exception and roadmap M5 due-Charge amendment are already in those documents. M4A–M4D **shipped and merged** on a pinned CI-green descendant of [`88db505`](https://github.com/tswarren/DepartureDesk/commit/88db505fa81ab740ad134fea28998841b401d7e9) (M4D merge). [M4D.0](../m4d0-narrow-group-departure-builder.md) must be **Accepted and shipped** (or an accepted M4E plan must explicitly name another base) before this gate runs. [M4A](../m4a-service-definitions-and-sources.md), [M4B](../m4b-client-pricing-and-anonymous-preview.md), [M4C](../m4c-packages-choices-and-client-terms.md), and [M4D](../m4d-publication-and-live-feasibility.md) are shipped. Verify accepted repository contracts, merged SHAs, findings, and CI before promoting this gate. Coding slices reconfirm required CI on the branch tip.

**Authority:** Parent, ADR 0014, and M4.0 as above. Shipped M3F fixture ledger and scenario narratives. Pin the actual M4D.0 merge (or later CI-green descendant that includes shipped M4D and M4D.0) and required workflow results when accepting M4E.

## Goal and scope

Prove that M4A–M4D.0 work **together** as a calm Staff journey: from Supplier planning through Client price, optional Package and choices, one review/publication action, live availability, pause/resume and successor review. Close finding-driven defects, measure friction against the accepted M4.0 **rules**, validate two ledger-labeled scenario journeys and the M5 handoff, then mark M4 complete only when all parent obligations have evidence.
...
| Vineyard Tour | June 5–7, 2027 operating dates, 30-seat coach, distinct Standard/Deluxe dinner Supplier Items, tasting and lunch shape. Hotel A/B sequence and nights remain shape-only. | Build distinct services, an inline package-only service (`owning_package_version_id` set; no `independently_sellable` column), a bundled Package price `P` evaluated for **one person** then doubled. Double is `2P`, not `4P`. The 100% supplement `s = 1.0` is **illustrative** and optional on ordinary bundles; a single example is `P + round_minor(P × s)` with no duplicate lodging charge. Dinner is an exactly-one **Client** choice with separate bound Supplier Items (one selected Item, not both); unresolved dinner rates stay unknown or use clearly labeled test values, never invented agreement facts. Shared coach cost is deduplicated across included services and makes margin unknown without a stated enrollment assumption. |
