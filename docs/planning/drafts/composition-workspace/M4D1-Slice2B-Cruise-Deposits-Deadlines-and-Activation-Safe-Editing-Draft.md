...
The canonical accepted 2B plan must be the sole implementation authority for typed Stop D chrome. Discovery notes and wireframes may guide layout but must not compete with this contract. Cabin-quantity and cumulative-target **domain** rules live in 2B-R / ADR 0013—not in this adapter plan.
...
Introduce or name definition-scoped detectors at Accept, for example:

- `DetectCruiseDepositRequirementShape`
- `DetectCruiseSupplierDeadlineShape`

Exact names are locked at Accept after inspecting shipped M3E services.

Typed compatibility requires:

- one supported semantic template;
- supported amount/timing/coverage shape (including 2B-R shapes when shipped);
...
Exact route helpers lock at Accept after code inspection. Preferred mutation approach:
...
**Cruise adapter read/preview services** (names locked at Accept):
...
Preferred typed route ownership:
...
One Accepted 2B contract (after 2B-R and 2A.2R3 ship), delivered through three vertical PRs:
...
The full UI/domain contract must be accepted before 2B-A begins so the first PR does not harden another command-correct but workflow-incomplete surface.

---

## 22. Decisions

### 22.1 Default ownership — locked for this draft
...
### 22.2–22.4 Cabin quantities and cumulative target — blocked on 2B-R

**Not Accepted in this adapter plan.** Product Path B (`$50 × initially blocked`, `$500 × retained`, source-aware cumulative) is locked in [Slice 2B-R](../../m4d1-slice2br-cruise-deposit-semantics-amendment.md). That slice amends ADR 0013 / M3E register and ships `capacity_pool_units` plus source-aware `cumulative_target`.

Until 2B-R ships, this plan must not claim those economics are ordinary typed-adapter work, and must not Accept.

### 22.5 Option date — locked for this draft
...
### 22.6 Commitment-line catalog — locked for this draft (Celebrity)
...
### 22.7 Legal names versus rooming list — locked for this draft

For the canonical fixture, create only the contracted **rooming-list** obligation. Do not invent a `legal_names_due` requirement.

`names_assigned_to_supplier` remains the final-deposit trigger only. On Accept of 2B-R / 2B, amend parent Stop D to drop the “legal names” bullet so parent and fixture agree.

### 22.8 Successor reconciliation — locked for this draft
...
### 22.9 Definition deletion — locked for this draft
...
### 22.11 Commands, routes, reorder, duplicate — locked for this draft
...
Stop D typed Celebrity proof requires **shipped 2B-R**. If 2B-R is missing, 2B must not claim support for “initially blocked cabins,” “retained cabins,” or `$500 × retained` cumulative calculation.
...
1. 2B-R and 2A.2R3 are shipped and Staff can construct the accepted Celebrity Deposit and Deadline definitions entirely through visible typed controls.
