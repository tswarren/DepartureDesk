...
23. The MVP amount catalog supports:
    ...
    - target amount less prior materialized Deposit Requirements (**staged balance / cumulative target**), including **quantity-derived targets** with **explicit contributing** prior Deposit Requirement Definitions (not only a single fixed target amount).
24. Celebrity staged deposits are **cumulative and capacity-sourced**: initial **$50 × initially blocked cabins**; final **$500 × retained cabins** cumulative target with **explicit** contributing prior definitions and **source-aware** credit/clamp. The remainder is not “an additional flat $500,” and it is not Arrangement-wide $500 − $50 when per-source quantities differ. The `names_assigned_to_supplier` milestone remains **Arrangement-wide**; scenario proof must not imply that assigning names for one cabin advances only that cabin’s deposit. Do not introduce Traveler or client-allocation records.
25. Cabin-block and retained-cabin deposit quantities use deposit **`quantity_basis = capacity_pool_units`**, reading authoritative Capacity Pool opening/projection facts for explicit coverage. This is not a new Pool `measurement_basis` (ADR 0010 catalog unchanged). Other deposits may continue to use `resource_units`, `traveler_positions`, or `explicit` quantity bases where appropriate. Silent fallback to usage-assumption units or “one per Resource” is forbidden for `capacity_pool_units`.
...
93. Reusable executable scenario builders provide slice and composite proof:
    - Celebrity cruise: **$50 × initially blocked cabins** initial deposit + **$500 × retained cabins** source-aware cumulative final deposit; Arrangement-wide `names_assigned_to_supplier` milestone (not per-cabin advancement); activation materializes final tranche with March 11 fallback Deadline; milestone replaces unelapsed Deadline; **rooming-list** Deadline only (no invented legal-names obligation in the canonical fixture);
