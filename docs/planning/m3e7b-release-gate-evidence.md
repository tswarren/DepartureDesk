...
**Status:** Recorded for PR #108 after composite scenario and UI proof hardening  
...
| --- | --- | --- |
| Celebrity Beyond (Nov 6–13, 2027): `resource_units` coverage for $50 per-cabin rate resolving Arrangement-wide to quantity 1; cumulative $500 → $450 remaining; March 11 fallback; milestone replacement without duplicate; 24-cabin inventory assumption drives cost/exposure; rooming-list (60 days → 2027-09-07) and legal-names (30 days → 2027-10-07); shared evidence coverage for two confirmation commitments; post-attestation cabin increment opens one new deposit commitment; gross vs commission vs net | Pass | `test/services/m3e7b_scenario_release_gate_test.rb` — Celebrity Beyond group-cruise test; slice depth also in `test/services/m3e3_deposit_requirements_milestones_test.rb` |
...
| Criterion | Result | Evidence |
| --- | --- | --- |
| Query bound (disposition list) | Pass | Bounded count in `m3e1a_commitment_lifecycle_test.rb` |
| Commitment list `EXPLAIN` at representative scale | Pass | `m3e7b_scenario_release_gate_test.rb` — ≥80 commitments; `enable_seqscan=off`; asserts `index_supplier_commitments_on_supplier_blocker`; rejects `Seq Scan on supplier_commitments` |
| Operational UI recovery (422, Viewer denial) | Pass | `test/controllers/m3e7a_ending_and_exposure_ui_request_test.rb` |
| M3E keyboard + responsive (375 / 768 / 1280 / 1400) | Pass | `test/system/m3e7b_operational_surfaces_accessibility_test.rb` — deposit/Deadline error-summary keyboard recovery; exposure, ending, deposit, and Deadline index surfaces at required viewports; ending preview invalid-path focus. Local Docker skips when Chrome is absent; CI `system-test` is blocking. |
...
| Plan, ADR 0013, decision register, roadmap, docs index, AGENTS | Pass | Marked Shipped with this evidence index linked from the M3E.7b slice |
...
- Celebrity cumulative deposit and `names_assigned_to_supplier` remain **Arrangement-wide**; the 24-cabin assumption drives cost/exposure calculation, not per-cabin deposit advancement. Resource-unit coverage for the $50 rate resolves to quantity 1 when the Resource has no scoped assumption.
...
./dev/rails-docker bin/rails test test/system/m3e7b_operational_surfaces_accessibility_test.rb
