...
- Ordinary create is Client title plus one source or an explicit fulfillment basis. Dates, provider, category, currency, and capacity context are read from the selected source. M3 cost and indicative margin are absent.
- Advanced multi-source topology is behind disclosure. Discard uses a confirmation that collects a reason (`draft → abandoned`). There is no Publish action. Forms use `#form-error-summary`, hidden `lock_version`, and server-issued idempotency keys.
