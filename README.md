...
The shipped domain is agency identity plus the Client directory through M1B, with M1C Supplier core implemented on the current branch and considered shipped when that branch merges: an agency workspace, agency users, invitations, sessions, offices as context, administrator maintenance of the profile, offices, and users, Client Person, Client Organization, person-backed and organization-backed Client, person-owned and organization-owned contact points including organization websites, effective-dated organization contacts, Supplier with fixed categories and Supplier-owned contact points including websites. [`docs/README.md`](docs/README.md) maps documentation authority; product authority is [`docs/planning/departure-desk-mvp.md`](docs/planning/departure-desk-mvp.md) and [`docs/planning/commercial-domain-decision-register.md`](docs/planning/commercial-domain-decision-register.md). Supplier Locations, Supplier Contacts, departures, and posted money objects are not implemented. [`docs/terminology.md`](docs/terminology.md) defines current vocabulary. PostgreSQL, Solid Queue, Docker development, UUIDv7 support, `money-rails`, and the application theme are present.
