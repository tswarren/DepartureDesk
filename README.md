...
The shipped domain is agency identity plus the complete M1 Client and Supplier directories: an agency workspace, agency users, invitations, sessions, offices as context, administrator maintenance of the profile, offices, and users, Client Person, Client Organization, person-backed and organization-backed Client, person-owned and organization-owned contact points including organization websites, effective-dated organization contacts, Supplier with fixed categories and Supplier-owned contact points including websites, Supplier Locations, Supplier Contacts, and Supplier Contact-owned email and phone destinations. M1E shipped directory proof and hardening without a new domain aggregate. M2 is complete: dated Departures, activation, `D-` issuance, search, departed transitions, scheduled departed jobs, schedule/currency/lifecycle corrections, and M2C proof and hardening. Travel Program remains deferred. [M3A](docs/planning/m3a-draft-arrangement-structure.md) ships draft Supplier Arrangement structure under Departures. [M3B](docs/planning/m3b-supplier-capacity.md) ships draft Item capacity configuration and the capacity event/projection/reconciliation engine without Arrangement activation or Staff event UI. [M3C](docs/planning/m3c-cost-terms-and-forecasts.md) ships draft Supplier cost terms and derived forecasts without Arrangement activation or effective contracted terms. [M3D.0](docs/planning/m3d0-planning-workspace-compression.md) ships guided Item and cost setup, bulk hierarchical capacity decisions, contextual assumptions, readiness review, and focused reorder modes over those M3A–M3C records. The [M3 parent](docs/planning/m3-supplier-planning.md) is accepted and amended through M3E authority. [M3D](docs/planning/m3d-activation-reservations-confirmations.md) is shipped; Arrangement activation, Reservations, confirmations, effective-capacity controls, and confirmation-triggered commitment openings are shipped. [M3E](docs/planning/m3e-supplier-operational-control.md) and [ADR 0013](docs/adr/0013-supplier-operational-commitments-deadlines-exposure-and-ending.md) are shipped through M3E.7b (including M3E.5R and M3D remediations): openings/dispositions, Deadlines, deposits/milestones, exposure, Needs attention, and Arrangement ending. M3E is fully shipped and production-ready. Travel Program, remittance, FX, and M3F remain unimplemented. Client demand, Travelers, Obligations, and Payments remain unimplemented. [`docs/README.md`](docs/README.md) maps documentation authority; product authority is [`docs/planning/departure-desk-mvp.md`](docs/planning/departure-desk-mvp.md) and [`docs/planning/commercial-domain-decision-register.md`](docs/planning/commercial-domain-decision-register.md). Posted ledger money objects are not implemented; M3C ships draft cost-definition amounts only. [`docs/terminology.md`](docs/terminology.md) defines current vocabulary. PostgreSQL, Solid Queue, Docker development, UUIDv7 support, `money-rails`, and the application theme are present.

Architecture decisions are indexed in [`docs/adr`](docs/adr/README.md). [ADR 0001](docs/adr/0001-money-and-currency.md) accepts `money-rails`; [M3C](docs/planning/m3c-cost-terms-and-forecasts.md) shipped the first draft Supplier cost monetary tables. [ADR 0005](docs/adr/0005-agency-identity.md) is the tenancy contract. [ADR 0006](docs/adr/0006-separate-identity-domains.md) governs the separation of planned identity domains. [ADR 0007](docs/adr/0007-departure-operational-root.md) accepts Departure as the dated operational root. [ADR 0008](docs/adr/0008-supplier-arrangement-version-topology.md) and [ADR 0009](docs/adr/0009-supplier-contracting-and-service-provider-roles.md) accept Arrangement topology and Supplier-role meanings; M3A draft topology and role behavior are implemented. [ADR 0010](docs/adr/0010-supplier-capacity-ledger-and-projection.md) accepts the capacity ledger; [M3B](docs/planning/m3b-supplier-capacity.md) shipped the implementing slice. [ADR 0011](docs/adr/0011-supplier-cost-definitions-and-forecast-evaluation.md) accepts cost definitions and forecast evaluation; [M3C](docs/planning/m3c-cost-terms-and-forecasts.md) shipped the implementing slice. [ADR 0012](docs/adr/0012-arrangement-activation-reservations-and-confirmations.md) accepts activation, Reservations, and confirmation evidence; [M3D](docs/planning/m3d-activation-reservations-confirmations.md) ships the implementing slice. [ADR 0013](docs/adr/0013-supplier-operational-commitments-deadlines-exposure-and-ending.md) accepts commitments, Deadlines, deposits, exposure, Needs attention, and Arrangement ending; [M3E](docs/planning/m3e-supplier-operational-control.md) ships the implementing slice. [ADR 0002](docs/adr/0002-agency-tenancy-and-membership.md) and [ADR 0003](docs/adr/0003-membership-lifecycle-and-invitations.md) are superseded by ADR 0005.
...
flowchart TD
    ...
    CT --> SC[Client Trip Services, Travelers, Charges, and Receipts]
```
...
| Supplier | The contracting commercial counterparty, such as a cruise line, hotel, motorcoach company, vineyard, insurer, airline, or other provider. |
| Client Trip Service | A distinct service selected, included, required, or added on a Client Trip, such as cruise passage, hotel stay, motorcoach seat, tasting, insurance, or air. |
...
| Client | A payer/responsibility identity based on exactly one Client Person or Client Organization. A Client is not necessarily a Traveler. |
| Client trip | The agency-facing commercial and operational record for one primary Client within a Departure. It can contain multiple Client Trip Services fulfilled by multiple Suppliers. |
| Traveler | A person who will travel. Travelers can share accommodations while retaining separate financial responsibility. |
| Payer | The actual source of a particular Receipt. A Payer need not be the primary Client, a Responsible Client, or a Traveler. |
...
| Household | A servicing and communication grouping of Client Persons. It is not automatically a cabin, Traveling Party, payment account, or insurance household. |
...
- A Client Trip may contain any number of Supplier-fulfilled Client Trip Services.
...
- Framework-owned Solid Queue tables may retain bigint identifiers. Active Storage is not installed.
...
System tests run in Chromium inside the development image, and in GitHub CI:

```bash
./dev/rails-docker bin/rails test:system
```

Rebuild the image with `docker compose build` if that command reports that Chromium is not installed.
